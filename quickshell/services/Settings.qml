...
QtObject {
    ...
    readonly property var _schema: [
        ...
        { key: "barStyle", type: "string", def: "glass" },
        ...
        for (let i = 0; i < _schema.length; i++) {
            ...
            if (item.type === "string") {
                ...
                try {
                    ...
                    if (obj && typeof obj === "object") root[item.key] = obj;
                } catch(e) {}
            }
        }
    }
    ...
    function toConf() {
        ...
        for (let i = 0; i < _schema.length; i++) {
            ...
            if (item.type === "string") {
                ...
                lines.push(item.key + "='" + JSON.stringify(val || {}) + "'");
            } else {
                lines.push(item.key + "=" + val);
            }
        }
    ...
    function resetToDefaults() {
        ...
        for (let i = 0; i < _schema.length; i++) {
            let item = _schema[i];
            root[item.key] = (item.type === "json") ? Object.assign({}, item.def) : item.def;
        }
        root._loading = false;
        root.save();
    }
