                ...
                RowLayout {
                    ...
                    Repeater {
                        model: [
                            ...
                            { id: "audio", label: "audio", icon: Theme.iconVolHigh },
                ...
                ColumnLayout {
                    ...
                    Rectangle {
                        ...
                        RowLayout {
                            ...
                            IconButton {
                                icon: (root.sink?.audio?.muted ?? false) ? Theme.iconVolMute : Theme.iconVolHigh
                                iconSize: Theme.fontSizeMd
                            ...
                            // fixing slider math so 150% doesn't desync with track width
                            Rectangle {
                                id: volTrack
                                Layout.fillWidth: true
                                height: 8
                                ...
                                Rectangle {
                                    width: parent.width * Math.min(1.0, Math.max(0.0, (root.sink?.audio?.volume ?? 0) / 1.5))
                                ...
                                MouseArea {
                                    ...
                                    function applySinkVolume(posX) {
                                        if (!root.sink?.audio || width <= 0) return
                                        const clamped = Math.max(0.0, Math.min(1.5, (posX / width) * 1.5))
                                        root.sink.audio.volume = clamped
                                        if (root.sink.audio.muted && clamped > 0) root.sink.audio.muted = false
                                    }
                    ...
                    Rectangle {
                        ...
                        RowLayout {
                            ...
                            IconButton {
                                icon: (root.source?.audio?.muted ?? false) ? Theme.iconMicMute : Theme.iconMic
                                iconSize: Theme.fontSizeMd
                            ...
                            Rectangle {
                                ...
                                Rectangle {
                                    width: parent.width * Math.min(1.0, Math.max(0.0, root.source?.audio?.volume ?? 0))
                                    height: parent.height
                                    radius: Theme.radiusPill
                                    color: Theme.primary
                                }
                                ...
                                MouseArea {
                                    ...
                                    function applyMicVolume(posX) {
                                        if (!root.source?.audio || width <= 0) return
                                        const clamped = Math.max(0.0, Math.min(1.0, posX / width))
                                        root.source.audio.volume = clamped
                                        if (root.source.audio.muted && clamped > 0) root.source.audio.muted = false
                                    }
